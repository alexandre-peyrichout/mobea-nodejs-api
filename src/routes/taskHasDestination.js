...
router.get("/:id", (req, res) => {
    ...
    connection.query("SELECT * from task_has_destination WHERE task_idtask = ? and destination_iddestination = ?", [task_idtask, destination_iddestination], (err, results) => {
        if (err) {
...
router.put("/:id/:id2", (req, res) => {
    ...
    connection.query("UPDATE task_has_destination SET ? WHERE task_idtask = ? and destination_iddestination = ?", [formData, idUrl, idUrl2], (err) => {
        if (err) {
...
router.delete("/:id/:id2", (req, res) => {
    ...
    connection.query("DELETE FROM task_has_destination WHERE task_idtask = ? and destination_iddestination = ?", [idUrl, idUrl2], (err) => {
        if (err) {
